...
async def tralka(client: Client, message: types.Message):
    ...
    paytext_splitted = paytext.split()

    if len(paytext_splitted) == 2:
        return await message.edit(
            generate(int(paytext_splitted[0]), int(paytext_splitted[1]), None)
        )

    else:
        return await message.edit(
            generate(
                int(paytext_splitted[0]), int(paytext_splitted[1]), paytext_splitted[2],
            ))


client.desc["Tralka"] = {
    "use": ".tralka <word_count> <caps_rate (in %)> <recepient name>",
    "description": "Bulling hard."
}